...
public class Worker : BackgroundService
{
    ...
    private readonly ConcurrentQueue<ConsumeResult<int, string>> concurrentQueue = new ConcurrentQueue<ConsumeResult<int, string>>();
    ...
    {
        Task consumeFromEventHub = Task.Run(() =>
       {
        ...
        Task consumeFromQueue = Task.Run(() =>
        {
            long count = 0;
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                while (!concurrentQueue.TryDequeue(out msg)) { }
        ...
        await Task.WhenAll(consumeFromEventHub, consumeFromQueue);
    }
}